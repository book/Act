...
for my $language (@languages) {
    ...
    for my $p (@$c) {
        isa_ok($p, 'HASH');
        like( $p->{iso}, qr/^[a-z]{2}$/, "$language iso $p->{iso}");
        ok($p->{name} && $p->{name} ne "country_$p->{iso}",  "$language $p->{iso} name");
        is(Act::Country::CountryName($p->{iso}), $p->{name}, "$language $p->{iso} name CountryName");
    }
}
