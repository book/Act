...
So, that's ten 5 minutes lightning talks per hour.
...
People with the 'orga' bit set can always see (and edit) the schedule. It
becomes public when the [talks] item named "show_schedule" is set to 1.

The first time you'll look at the schedule, it'll be ugly : all the
talks are schedule to a non existing room at the same time. This will
change once you assigned a room to each talk.

The HTML table uses as many styles as you have rooms, plus three. Imagine
you have three rooms, the table will use the following styles: header,
out, venue, r1, r2 and r3. Please note that you do not need to define
your own CSS for this: simply load the style located at /css/schedule.css
with E<lt>link rel="stylesheet" type="text/css" href="/css/schedule.css" /E<gt>.

We'll add new colours when a conference has more than three rooms.
...
Rooms 'out' and 'venue' are special in the sense that events occuring
...
Only hour-long tutorial should be cut by events.

There are some cases for which Act does not display the schedule
...
anything happening outside the conference (pub crawls, etc.)
...
Lightning talks sessions are currently not fully handled by Act. One
can submit a lightning talk, which will be accepted or not, as for the
other talks. But you'll need to create one or several "lightning talks"
events in which you'll paste the list of lightning talks. We should
probably fix this.
