...
for template files.
When a new template is created (C<< Act::Template->new >>), the INCLUDE_PATH
...
C<< Act::Template->new >> acts as an object factory: it maintains an internal
...
C<< <t>...</t> >> surrounds a multilingual text section.
C<< <xx>...</xx> >> surrounds a particular language version, where C<xx> is a language
...
This is a convenience method. C<< $template->variables() >> will accumulate template
