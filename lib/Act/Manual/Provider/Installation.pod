...
Act::Manual::Provider::Installation - Installing Act on your system

=head1 Installing Act
...
Next, create a local configuration file in F<$ACTHOME/conf/local.ini>.
This file is not part of the distribution: its contents override
the global settings in F<$ACTHOME/conf/act.ini>. If you have no need
to override these standard settings, local.ini isn't needed.

You are now ready to install and configure the database system.
See the details in database/INSTALL.E<lt>dbE<gt> (where E<lt>dbE<gt> is one of
Pg, MySQL....)
...
If you plan to modify some default templates, you should copy/paste*
them to the local templates/ directory.
